...
config-local-username() {
    ...
    if [[ ! -z ${REPO_NAME} && ${REPO_NAME} == 'exercise' ]]; then
        git config --local user.name "git-tasks variant bot"
        git config --local user.email "example@variant.no"
...
pre-setup () {
    task="$(basename $(pwd))"   # task: name of the exercise which is respective folder name
